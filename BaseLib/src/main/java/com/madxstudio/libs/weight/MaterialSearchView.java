...
import com.madxstudio.libs.tools.AnimationUtil;
...
public class MaterialSearchView extends FrameLayout implements Filter.FilterListener {
    ...
    private PopupWindow mSuggestionsWindow;
    ...
    private void initStyle(AttributeSet attrs, int defStyleAttr) {
        TypedArray a = mContext.obtainStyledAttributes(attrs, R.styleable.MaterialSearchView,
                defStyleAttr, 0);
        ...
        if (a != null) {
            ...
            if (a.hasValue(R.styleable.MaterialSearchView_searchSuggestionBackground)) {
                setSuggestionBackground(a.getDrawable(R.styleable
                        .MaterialSearchView_searchSuggestionBackground));
            }

            if (a.hasValue(R.styleable.MaterialSearchView_searchSuggestionIcon)) {
                setSuggestionIcon(a.getDrawable(R.styleable
                        .MaterialSearchView_searchSuggestionIcon));
            }

            if (a.hasValue(R.styleable.MaterialSearchView_android_inputType)) {
                setInputType(a.getInt(R.styleable.MaterialSearchView_android_inputType,
                        mSearchSrcTextView.getInputType()));
            }

            if (a.hasValue(R.styleable.MaterialSearchView_searchMsgIcon)) {
                setSearchIcon(a.getDrawable(R.styleable.MaterialSearchView_searchMsgIcon));
            }
    ...
    private void initiateView() {
        ...
        mSearchSrcTextView = (EditText) mSearchLayout.findViewById(R.id.search_text_view);
        mBackBtn = (ImageButton) mSearchLayout.findViewById(R.id.action_up_btn);
        mSearchIcon = (ImageButton) mSearchLayout.findViewById(R.id.search_icon);
        mVoiceBtn = (ImageButton) mSearchLayout.findViewById(R.id.action_voice_btn);
        mEmptyBtn = (ImageButton) mSearchLayout.findViewById(R.id.action_empty_btn);

        View searchView = inflater.inflate(R.layout.layout_suggestions, null, false);
        mSuggestionsWindow = new PopupWindow(searchView, ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.WRAP_CONTENT);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            mSuggestionsWindow.setBackgroundDrawable(mContext.getDrawable(R.drawable.popup_shadow));
        } else {
            mSuggestionsWindow.setBackgroundDrawable(mContext.getResources().getDrawable(R
                    .drawable.popup_shadow));
        }
        mSuggestionsWindow.setOutsideTouchable(true);
        mSuggestionsWindow.setFocusable(false);
        mSearchSrcTextView.setOnKeyListener(new OnKeyListener() {
            @Override
        ...
        mSuggestionsListView = (ListView) searchView.findViewById(R.id.suggestion_list);
    ...
    public void loadSuggestions() {
        suggestions = PreferencesUtil.getValue(Constants.KEY_SUGGESTIONS, new HashSet<String>());
    ...
    private void initSearchView() {
        ...
        mSearchSrcTextView.addTextChangedListener(textWatcher);
    ...
    private void startFilter(CharSequence s) {
        if (needFilter && mAdapter != null && mSearchLayout.getVisibility() == VISIBLE) {
    ...
    private final OnClickListener mOnClickListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            if (v == mBackBtn) {
                ...
                showSuggestions();
                mSearchSrcTextView.setText(mSearchSrcTextView.getText());
    ...
    private void onVoiceClicked() {
        ...
        //intent.putExtra(RecognizerIntent.EXTRA_PROMPT, "Speak an item name or number");    //
        // user hint
        intent.putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL, RecognizerIntent
                .LANGUAGE_MODEL_WEB_SEARCH);    // setting recognition model, optimized for short
        // phrases – search queries
        intent.putExtra(RecognizerIntent.EXTRA_MAX_RESULTS, 1);    // quantity of results we want
        // to receive
    ...
    private boolean isVoiceAvailable() {
        ...
        List<ResolveInfo> activities = pm.queryIntentActivities(new Intent(RecognizerIntent
    ...
    public void setCursorDrawable(int drawable) {
        try {
            // https://github.com/android/platform_frameworks_base/blob/kitkat-release/core/java
            // /android/widget/TextView.java#L562-564
    ...
    /**
     * Call this method to show suggestions list. This shows up when adapter is set. Call
     * {@link #setAdapter(SearchAdapter)} before calling this.
     */
    public void showSuggestions() {
        if (mAdapter != null && mAdapter.getCount() > 0 && !mSuggestionsWindow.isShowing()) {
            mSuggestionsWindow.showAsDropDown(mSearchLayout);
        }
    }
        ...
        if (suggestions != null && suggestions.size() > 0) {
            ...
            setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    ...
                    if (position == count - 1) {
                        adapter.setSuggestions(new HashSet<String>());
                        mSearchSrcTextView.setText(null);
                        PreferencesUtil.saveValue(Constants.KEY_SUGGESTIONS, new HashSet<String>());
                    } else {
    ...
    public boolean dismissSuggestions() {
        if (mSuggestionsWindow.isShowing()) {
            mSuggestionsWindow.dismiss();
            return true;
        } else {
            return false;
        }
    ...
    public void displayQuery(CharSequence query) {
        // mSearchSrcTextView.clearFocus();
        mSearchSrcTextView.removeTextChangedListener(textWatcher);
     ...
     * this method call has not effect.
     *
     * @param show
     */
